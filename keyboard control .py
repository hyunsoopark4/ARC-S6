...
class Robot():
    ...
    def get_inverse_kinematic(self, p, pc_ip="192.168.100.200", pc_port=30010):
        command = f'''
def test():
...
'''
        conSuc, sock = self.connectETController(self.ip, 30001)
        if conSuc:
            try:
    ...
    def get_forward_kinematic(self, p, pc_ip="192.168.100.200", pc_port=30010):
        command = f'''
def test():
    socket_open("{pc_ip}", {pc_port})
    sleep(0.5)
...
class Robot_30001():
    ...
    def RecvPopup(self, port):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            s.listen()
            print(f"Listening for robot on port {self.port}...")
            conn, addr = s.accept()
            with conn:
                print(f"Connected by {addr}")
                while True:
...
if __name__ == "__main__":
    print("=== ROBOT ABSOLUTE CONTROL START ===")

    # 1. 로봇 통신 객체 생성
    robot_send = ROBOT_SEND()

    # 2. 상태 수신 쓰레드 (백그라운드 실행)
    ...
    try:
        ...
        # 초기 속도 설정
        current_speed = 30
        robot_send.robot_speed(current_speed)
        print(f">>> 초기 속도 {current_speed}% 설정 완료")
        time.sleep(1)
        ...
        # ★ HOME 포지션 정의 (관절 1~6, 라디안 값)
        # 예시: [0, -90, 90, 0, 90, 0] 도를 라디안으로 변환한 값
        HOME_POSITION = [0.0, -1.5708, 1.5708, 0.0, 1.5708, 0.0]
        
        # 현재 목표 관절 데이터 (시작 시 Home 위치로 가정)
        # ※ 실제 로봇의 현재 위치를 받아오면 더 좋지만, 안전을 위해 Home으로 이동 후 시작합니다.
        current_joints = list(HOME_POSITION) 

        print(f"\n[INIT] 안전을 위해 초기 위치(HOME)로 이동합니다.")
        joint_str = "[" + ", ".join([f"{x:.4f}" for x in current_joints]) + "]"
        robot_send.send_command(f"movej({joint_str}, a=0.5, v=0.3)")
        time.sleep(5)
        ...
        while True:
            print("\n" + "="*50)
            # 현재 각도 상태 출력 (라디안 -> 도 변환)
            degrees_now = [round(math.degrees(rad), 1) for rad in current_joints]
            print(f"📍 현재 각도: {degrees_now}")
            print(f"🚀 현재 속도: {current_speed}%")
            print("-" * 50)
            print(" [1~6] 관절 선택 및 각도 입력")
            print(" [h]   HOME 위치로 복귀")
            print(" [s]   속도(Speed) 조절")
            print(" [q]   종료")
            print("="*50)
            
            menu = input("명령 입력 >> ").strip().lower()
            ...
            # 2. HOME 복귀
                ...
                print("🏠 HOME 위치로 복귀합니다...")
                current_joints = list(HOME_POSITION) # 목표를 홈으로 변경
                
                # 명령 전송
                joint_str = "[" + ", ".join([f"{x:.4f}" for x in current_joints]) + "]"
                robot_send.send_command(f"movej({joint_str}, a=0.5, v=0.5)")
                time.sleep(3) # 이동 대기

            # 3. 속도 조절
            elif menu == 's':
                try:
                    new_speed = int(input(">>> 설정할 속도 (1~100) 입력: "))
                    if 0 < new_speed <= 100:
                        current_speed = new_speed
                        robot_send.robot_speed(current_speed)
                        print(f"✅ 속도가 {current_speed}%로 설정되었습니다.")
                    else:
            ...
            # 4. 관절 제어 (1~6번)
                ...
                joint_idx = int(menu) - 1 # 리스트 인덱스는 0부터 시작하므로 -1
                
                try:
                    # 목표 각도 입력 받기
                    target_deg_str = input(f">>> J{joint_idx+1}의 목표 각도(도) 입력 (-180 ~ 180): ")
                    target_deg = float(target_deg_str)

                    # 입력값 업데이트 (도 -> 라디안)
                    current_joints[joint_idx] = math.radians(target_deg)

                    # 이동 명령 생성 및 전송
                    joint_str = "[" + ", ".join([f"{x:.4f}" for x in current_joints]) + "]"
                    # v값을 0.3~0.8 정도로 주면 부드럽게 이동합니다.
                    robot_send.send_command(f"movej({joint_str}, a=0.8, v=0.5)")
                    
                    print(f"🚀 J{joint_idx+1} 관절을 {target_deg}도로 이동 중...")
                    
                except ValueError:
                    print("❌ 올바른 각도 숫자를 입력하세요.")
